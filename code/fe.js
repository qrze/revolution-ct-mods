...
var getPublicationMultiplier = (tau) => tau.pow(tauMultiplier) * BigNumber.FIVE;
var getPublicationMultiplierFormula = (symbol) => "5" + symbol + "^{0.331}";
var getTau = () => currency.value.pow(tauMultiplier);
var getCurrencyFromTau = (tau) => [tau.max(BigNumber.ONE).pow(1 / (1*tauMultiplier)), currency.symbol];
